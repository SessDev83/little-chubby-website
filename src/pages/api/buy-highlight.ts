...
export const POST: APIRoute = async ({ request, cookies }) => {
  ...
  try {
    ...
    // Rate limit: max 5 boost-like purchases per hour (reuse 'boost' bucket).
    ...
    const { data: result, error: rpcErr } = await svc.rpc("purchase_highlight", {
      p_user_id: user_id,
      p_review_id: review_id,
      p_cost: HIGHLIGHT_COST,
      p_duration_days: HIGHLIGHT_DAYS,
    });
    ...
    await trackServerConversionEvent(svc, {
      ...
      userId: user_id,
      props: {
        item_type: "review_highlight",
        review_id,
        cost: HIGHLIGHT_COST,
        boost_id: res.boost_id ?? null,
